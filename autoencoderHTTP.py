...
tls_data = np.array(raw_data).astype(np.uint8)
#print(tls_data.shape)
...
not_http_data = np.concatenate((tls_data, other_data), axis=0)



#fase tuning iperparametro attraverso validation set
plt.plot(tf.losses.mae(x_val, model.predict(x_val)))
plt.plot(tf.losses.mae(not_http_data, model.predict(not_http_data)))
plt.show()
...
loss_not_http = tf.losses.mae(not_http_data, model.predict(not_http_data))

plt.plot(loss_x_test)
plt.plot(loss_not_http)
plt.show()



FN = np.sum(loss_x_test > THRESHOLD)
TN = np.sum(loss_not_http > THRESHOLD)
TP = len(loss_x_test) - FN
FP = len(loss_not_http) - TN
...
model.save('autoencoderHTTP_model.keras')